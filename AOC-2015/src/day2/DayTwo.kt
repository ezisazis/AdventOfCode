...
fun main(args: Array<String>) {
    val input = File("src/day2/day_two_input.txt").readLines()
    val presents = input
            .map { it.split("x") }
            .map { Present(it[0].toInt(), it[1].toInt(), it[2].toInt()) }
    
    val total: Int = presents.sumBy { it.getSurface() }
    println("Total square feet is $total")

    val ribbon: Int = presents.sumBy { it.getRibbonSize() }
    println("Total ribbon feet is $ribbon")
}

data class Present (val length: Int, val width: Int, val height: Int) {

    fun getSurface(): Int {
        return 2 * length * width + 2 * width * height + 2 * height * length + minOf(length * width, width * height, height * length)
    }
    ...
    fun getRibbonSize(): Int {
        return (length + width + height - maxOf(length, width, height)) * 2 + length * width * height
    }
